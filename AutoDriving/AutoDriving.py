...
def weighted_img(img, initial_img, q = 0.8, w = 1., e = 0.):
    """
    ...
    """
    return cv2.addWeighted(initial_img, q, img, w, e)
...
class AsyncTask:
    ...
    def TaskA(self):
        send_cmd(cmd0, ser)
        val0 = receive_cmd(cmd0, ser)
        send_cmd(cmd1, ser)
        val1 = receive_cmd(cmd1, ser)
        send_cmd(cmd2, ser)
        val2 = receive_cmd(cmd2, ser)
        send_cmd(cmd3, ser)
        val3 = receive_cmd(cmd3, ser)
        send_cmd(cmd4, ser)
        val4 = receive_cmd(cmd4, ser)
